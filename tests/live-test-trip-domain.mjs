...
async function main() {
  ...
  const tests = await page.evaluate(() => {
    ...
    // Trip domain tests
    results.push({ name: 'TripDomain loaded', pass: typeof window.TripDomain !== 'undefined' });
    results.push({ name: 'TripDomain.tripStatus', pass: typeof window.TripDomain.tripStatus === 'function' });
    results.push({ name: 'TripDomain.getTrip', pass: typeof window.TripDomain.getTrip === 'function' });
    
    // Test tripStatus logic
    const pastTrip = { start: '2020-01-01', end: '2020-01-05' };
    const upcomingTrip = { start: '2099-01-01', end: '2099-01-05' };
    const noDateTrip = {};
    
    results.push({ name: 'tripStatus past', pass: window.TripDomain.tripStatus(pastTrip)[0] === 'past' });
    results.push({ name: 'tripStatus upcoming', pass: window.TripDomain.tripStatus(upcomingTrip)[0] === 'upcoming' });
    results.push({ name: 'tripStatus no date', pass: window.TripDomain.tripStatus(noDateTrip)[0] === 'planned' });
    
    return results;
  });

  console.log('\n=== Trip Domain Live Test Results ===');
  let passed = 0;
  tests.forEach(t => {
    const status = t.pass ? '✓' : '✗';
