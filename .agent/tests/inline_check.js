  ...
  async function openGuestTrip(token){
    try{
      ...
      lockNavForGuest();
      renderGuestView(data, guestSession.isMember);
      show('guestView');
  ...
  function renderGuestView(t, isMember){
    $('guestTripName').textContent = t.name || 'Trip';
    $('guestTripMeta').textContent = [t.destination, t.start_date||'', t.end_date||''].filter(Boolean).join(' · ') || '';
    const memberCount = (t.members||[]).length;
    const limit = t.participant_limit;
    const countText = limit
      ? `${memberCount} / ${limit} orang`
      : `${memberCount} orang`;
    $('guestParticipantSummary').innerHTML = '<span class="count">' + countText + '</span> sudah bergabung';
    ...
    if(isMember){
      ...
      renderGuestParticipantList(t);
      renderGuestLocationActions(t);
      if(previewText) previewText.innerHTML = '<div class="empty">Kamu ikut sebagai <b>' + esc(colState.name || 'Guest') + '</b>. Berikut itinerary trip ini.</div>';
    } else {
  ...
  // Render participant list (post-join)
  function renderGuestParticipantList(t){
    const el = $('guestParticipantList');
    if(!el) return;
    const members = (t.members||[]).filter(m => m.display_name);
    if(!members.length){
      el.innerHTML = '<div class="empty">Belum ada peserta lain.</div>';
      return;
    }
    el.innerHTML = '<div class="section-head" style="margin:18px 0 10px"><h2 style="font-size:18px">Peserta</h2></div>' +
      members.map(m => '<div class="agenda-item"><div class="agenda-body"><div class="agenda-title">' + esc(m.display_name) + '</div></div></div>').join('');
  }

  // Render location actions (post-join, opt-in only)
  function renderGuestLocationActions(t){
    const el = $('guestLocationActions');
    if(!el) return;
    // Location sharing only available if journey is active (same as member flow)
    // For now, show a disabled hint — actual share button appears after creator starts journey
    el.innerHTML = '<div class="empty" style="font-size:13px">Pembuat trip harus memulai Journey Mode agar kamu bisa berbagi lokasi.</div>';
  }
  ...
  function renderGuestItinerary(t){
    ...
    if(!el) return;
    if(list.length){
      el.innerHTML = list.map(item=>`<article class="agenda-item"><div class="agenda-when">${esc(item.date||'')} ${esc(item.time||'')}</div><div class="agenda-body"><div class="agenda-title">${esc(item.title)}</div>${item.note?`<div class="agenda-note">${esc(item.note)}</div>`:''}${item.link?`<a class="agenda-link" href="${esc(item.link)}" target="_blank" rel="noopener">Buka referensi →</a>`:''}${Number(item.budget)?`<div class="agenda-budget">${money(item.budget)}</div>`:''}</div></article>`).join('');
    } else {
      el.innerHTML = '<div class="empty">Belum ada agenda.</div>';
    }
    const exp = t.expenses||[];
    const exEl = $('guestExpenses');
    if(exEl) exEl.innerHTML = exp.length
      ? `<div style="font-size:13px;color:var(--muted)">${esc(String(exp.length))} catatan pengeluaran · total ${money(exp.reduce((s,x)=>s+(Number(x.amount)||0),0))}</div>`
      : '<div style="font-size:13px;color:var(--muted)">Tidak ada pengeluaran tercatat.</div>';
  }
...
colState.activeDate=null;state.pendingToGoId=null;$('selectedToGo').style.display='none';if(event&&event.target)event.target.reset();await openGroup(g.id,true);if(!colState.group||colState.group.id!==g.id){alert('Error: Gagal membuka grup. Silakan coba buat ulang.');return}history.replaceState({},'',groupLink(g.id));if(pendingToGoId){const sp=(state.toGo||[]).find(t=>t.id===pendingToGoId);if(sp){await API.addItem({group_id:g.id,date:start,title:sp.name,time:'',link:normalizeLink(sp.link||''),note:sp.note||'',budget:0,created_by:uid});await loadShared(g.id )}}}finally{freeBtn(submitBtn)}}
  ...
  async function joinGroup(id){
    ...
    if(!g){
      dbg('joinGroup: group not found via list_my_groups (not yet a member)');
      // Fall through: try isMember + join_group RPC directly for new members
    }
    else dbg('joinGroup: group found '+g.name);
  ...
  async function loadMembers(id){
    ...
    showLoading(ml, 'Memuat anggota...');
    const { data }=await API.getMembers(id);
    colState.members=data||[];colState.nameMap={};data.forEach(m=>colState.nameMap[m.user_id]=m.display_name);
    renderGroupPlanner();
  }
  ...
  async function renderJourneyView(){
    ...
    if(locRes && locRes.error && locRes.error.message && locRes.error.message.includes('no active journey')){
      ...
      colState.crewLocations = [];
    } else if(locRes && locRes.error && locRes.error.message && locRes.error.message.includes('location permission not granted')){
  ...
  async function loadCrewMap(){
    ...
    if(map && empty) showLoading(map, 'Memuat lokasi grup...');
  ...
  async function initJourneyRealtime(){
    ...
    var ch = sb.channel('member_locations:' + gid);
  ...
  function applyPermsUI(){
    ...
    if(addAgendaPanel) addAgendaPanel.style.display = canEditTrip() ? '' : 'none';
    const addExpensePanel=document.getElementById('expensesPanel');
    if(addExpensePanel) addExpensePanel.style.display = canEditTrip() ? '' : 'none';
